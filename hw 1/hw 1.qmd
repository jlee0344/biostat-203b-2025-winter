...
**(4.1) Solution: ** The 'wget -nc' command is used to download a file from the internet. The '-nc' flag is used to prevent the download of the file if it already exists in the current directory. If the file already exists, the download will not be executed. 
...
There is an additional way to count the number of words (one line may contain the same name multiple times): 
...
I manually created middle.sh, set the output to false, and include middle.sh in the submission folder.
...
Using `chmod` to make the file executable by the owner, and run the following command to test the script: 
...
**Solution: ** 
The output showcases that the release date of the book "Price and Prejudice" was May 9, 2013 (eBook $42671). 
It also shows that the language is English. 

Explain the meaning of `"$1"`, `"$2"`, and `"$3"` in this shell script.

**Solution: ** In this shelll script, the `"$1"` refers to the first argument passed to the script, which is the filename. The  `"$2"` refers to the second argument passed to the script, which is the end_line. The `"$3"` refers to the third argument passed to the script, which is the num_lines. 

Why do we need the first line of the shell script?

**Solution: ** We need the first line of the spell script beause it is a shebang line which is used to interpret the script. It tells the system which interpreter to use to execute the script. In this case, the shebang line is #!/bin/sh which tells the system to use the Bourne shell to interpret the script. 
...
```{bash}
mv "/Users/julielee/Desktop/203b-hw/hw 1/screenshot_2025-01-20.png" screenshot_2025-01-20.png
```

![Section 4.1.5](/Users/julielee/Desktop/203b-hw/hw 1/screenshot_2025-01-20.png)
