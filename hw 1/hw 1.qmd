...
The run times of `zcat < ~/mimic/labevents.csv.gz | wc -l`. The run time of `wc -l labevents.csv`.
The trade off between storage and speed for big data file is that 
...
**(4.1) Solution: ** The 'wget -nc' command is used to download a file from the internet. The '-nc' flag is used to prevent 
...
**(4.2) Solution: ** The first command 
The second command 
