...
```{r}
# Measure time & memory for selective column ingestion
...
file_path_gz <- "labevents.csv.gz"
file_path_csv <- "labevents.csv"

if (!file.exists(file_path_csv)) {
  cat("\n=== Decompressing 'labevents.csv.gz' to 'labevents.csv' ===\n")
  system(paste("gunzip -c", file_path_gz, ">", file_path_csv))  # Decompress
}

# Step 2: Measure ingestion + selection + filtering time
cat("\n=== Using Apache Arrow for Efficient Data Processing ===\n")

time_arrow <- system.time({
  # Open dataset with Arrow
  ds <- open_dataset(file_path_csv, format = "csv")

  # Select columns and filter rows using dplyr
  result <- ds %>%
    select(subject_id, itemid, charttime, valuenum) %>%  # Select relevant columns
    filter(itemid == 50868) %>%  # Filter for specific itemid (Example: Q2.3 condition)
    collect()  # Convert to tibble for further use
})

# Step 3: Display results
cat("\n=== Ingestion, Selection & Filtering Time ===\n")
cat("Elapsed Time (seconds):", time_arrow["elapsed"], "\n")

# Display the number of rows and first 10 rows
cat("\nNumber of Rows After Filtering:", nrow(result), "\n")
print(head(result, 10))  # Show first 10 rows
