...
# Function to test reading speed, memory, and data types
test_reading <- function(func, name) {
  time <- system.time({
    data <- func(file_path)
  })
  size <- object_size(data)
  types <- sapply(data, class)
  list(
    Method = name,
    Time = time["elapsed"],
    Memory = format(size, units = "auto"),
    DataTypes = paste(names(types), ":", types, collapse = ", ")
  )
}

# Perform tests
results <- list(
  test_reading(function(x) read.csv(x, stringsAsFactors = FALSE), "read.csv (base R)"),
  test_reading(read_csv, "read_csv (tidyverse)"),
  test_reading(fread, "fread (data.table)")
)
...
**(1.1 Solution) ** 
The function that is the fastest in R for reading a plain text data file, more specifically reading the compressed csv file, 'admsissions.csv.gz', is the 'fread' function in the data.table package. 

The amount of memory that each resuldtant data frame or tibble uses 
...
```{r, eval = FALSE}
df <- read_csv(file_path_labevents, col_select = c("subject_id", "itemid", "charttime", "valuenum"))
head(df)
```
**(2.2) Solution ** 
...
```
Checking the First 10 Lines of the New File 
```{bash}
...
Measuring read_csv Ingestion Time in R (using readr package) 
```{r}
# Measure time taken to read the filtered CSV file
start_time <- Sys.time()

# Read the filtered dataset
df_filtered <- read_csv("labevents_filtered.csv.gz")
