...
#Decompressing to my Directory 
gzip -dkf /home/jlee0344/mimic/hosp/labevents.csv.gz -c > ~/labevents.csv
...
```{bash}
# Measure the time for ingesting, selecting, and filtering
start_time <- Sys.time()

# Open the dataset with Apache Arrow
...
cat(sprintf("Time taken for ingest + select + filter: %.2f seconds\n", as.numeric(time_taken)))

# Display the number of rows
num_rows <- nrow(filtered_data)
cat(sprintf("Number of rows in the filtered dataset: %d\n", num_rows))

# Display the first 10 rows of the resulting tibble
