...
**Solution 1.1** 
The function that is the fastest is the 
There is a difference in the default parsed data types. 
The amount of ememory that each resultant dataframe or tibble use is 
...
file <- "~/mimic/hosp/admissions.csv.gz"

# 1. Measure time and memory for read.csv
base_r_time <- system.time({
  base_r_data <- read.csv(file)
})
base_r_memory <- object_size(base_r_data)

# 2. Measure time and memory for read_csv (tidyverse)
tidyverse_time <- system.time({
  tidyverse_data <- read_csv(file)
})
tidyverse_memory <- object_size(tidyverse_data)

# 3. Measure time and memory for fread (data.table)
data_table_time <- system.time({
  data_table_data <- fread(file)
})
data_table_memory <- object_size(data_table_data)

# Print results
cat("Results:\n")
cat("1. Base R (read.csv):\n")
cat("   Time: ", base_r_time["elapsed"], " seconds\n")
cat("   Memory: ", base_r_memory, " bytes\n\n")

cat("2. Tidyverse (read_csv):\n")
cat("   Time: ", tidyverse_time["elapsed"], " seconds\n")
cat("   Memory: ", tidyverse_memory, " bytes\n\n")

cat("3. Data.table (fread):\n")
cat("   Time: ", data_table_time["elapsed"], " seconds\n")
cat("   Memory: ", data_table_memory, " bytes\n\n")
EOF

Rscript test_read_speed.R
```
