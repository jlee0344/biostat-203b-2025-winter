...
The read.csv (Base R) by default, converts character columns into factors. 
```{r}
cat("Base R (read.csv):\n")
str(df_base)
```

The read_csv Tidyverse function does not convert character columns to factors by default. 
```{r}
cat("\nTidyverse (read_csv):\n")
str(df_tidy)
```

The fread (Data.Table) function automatically detects the approprotiates column types (numeric, character, etc) 
```{r}
cat("\nData.table (fread):\n")
str(df_dt)             
```
...
#Run Times 
time_tidy_coltypes <- system.time(
  df_tidy_coltypes <- read_csv(file_path, col_types = col_types)
)

#Memory Usage 
size_tidy_coltypes <- object_size(df_tidy_coltypes)

# Print runtime and memory usage
cat("Runtime (Elapsed Time):\n")
print(time_tidy_coltypes["elapsed"])

cat("\nMemory usage (bytes):\n")
print(size_tidy_coltypes)

```


**(1.2) Solution ** 
When we re-ingest admissions.csv.gz with specified column types, we are able to see that the file size remains 19M, the time taken to read the file is 2.413394 seconds, and the memory usage of the resulting tibble is 60.53 MB. 
...
time_taken <- microbenchmark(
  df <- read_csv(file_path_labevents),  
  times = 1  
)
print(time_taken)
head(df)
...
Second attempt using the provided R Server: Attemping to ingest 'labevents.csv.gz' using read_csv() (from the readr package) in R did not crash and was able to showcase the first 6 rows of the file "labevents.csv.gz". 
...
```{r}
# Measure ingestion time while selecting only the required columns
time_taken <- microbenchmark(
  df_only_columns <- read_csv(file_path_labevents, col_select = c("subject_id", "itemid", "charttime", "valuenum")),
  times = 1
)

# Print time taken and preview first few rows
print(time_taken)
head(df_only_columns)
```

**(2.2) Solution ** 
By selecting only 4 columns (subject_id, itemid, charttime, and valuenum) 
...
zcat ~/mimic/hosp/labevents.csv.gz | awk -F, '
    BEGIN { OFS = "," }
    NR == 1 || ($2 == 50912 || $2 == 50971 || $2 == 50983 || $2 == 50902 || $2 == 50882 || $2 == 51221 || $2 == 51301 || $2 == 50931) {
        print $1, $2, $3, $4
    }
...
# Define file paths
INPUT_FILE=~/mimic/hosp/labevents.csv.gz
OUTPUT_FILE=labevents_filtered.csv.gz

# Start timing the extraction process
START_TIME=$(date +%s)

# Extract required columns and filter for specified itemid values
zcat < "$INPUT_FILE" | awk -F',' '
    ...
    NR>1 && ($col["itemid"] ~ /^(50912|50971|50983|50902|50882|51221|51301|50931)$/) {
        print $col["subject_id"], $col["itemid"], $col["charttime"], $col["valuenum"];
    }
' | gzip > "$OUTPUT_FILE"

# End timing the extraction process
END_TIME=$(date +%s)
echo "Time taken to filter and compress: $((END_TIME - START_TIME)) seconds"

# Display first 10 lines of the new file
echo "First 10 lines of the filtered file:"
zcat < "$OUTPUT_FILE" | head -10

# Count the number of lines excluding the header
LINE_COUNT=$(zcat < "$OUTPUT_FILE" | wc -l)
echo "Number of lines (excluding header): $((LINE_COUNT - 1))"

# Start timing the read process
READ_START_TIME=$(date +%s)

# Measure read time by simply counting lines
zcat < "$OUTPUT_FILE" | wc -l > /dev/null

# End timing the read process
READ_END_TIME=$(date +%s)
echo "Time taken to read file: $((READ_END_TIME - READ_START_TIME)) seconds"
