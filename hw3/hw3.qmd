...
ggplot(admissions_summary, aes(x = num_admissions)) +
  ...
  scale_x_continuous(trans = "log1p", breaks = c(1, 2, 3, 5, 10, 15, 20, 30, 50, 100, 200)) +  
...
ggplot(mimic_icu_cohort, aes(x = marital_status, y = los, fill = marital_status)) +
  ...
  coord_cartesian(ylim = c(0, 25))  # Limits y-axis from 0 to 25
...
mimic_icu_cohort <- mimic_icu_cohort %>%
  ...
  filter(!is.na(los_category))  # Remove NA values
...
mimic_long <- mimic_icu_cohort %>%
  ...
  filter(!is.na(Value))  # Ensure lab values are not NA
...
mimic_long <- mimic_long %>%
  ...
  filter(Value >= y_min & Value <= y_max)  # Keep only minor outliers
