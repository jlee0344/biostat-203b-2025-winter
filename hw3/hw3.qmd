...
stay_ids <- subject_stays$stay_id
...
# Extract the first storetime per subject_id, stay_id, and itemid
first_store_per_stay <- chartevents_filtered %>%
  group_by(subject_id, stay_id, itemid) %>%  # Group by subject, stay, and item
  arrange(storetime) %>%  # Sort by storetime in ascending order
  slice_min(order_by = storetime, n = 1) %>%  # Select the first storetime per stay and item
  ungroup()

# Step 2: Join back and remove storetime after selection
chartevents_filtered <- chartevents_filtered %>%
  inner_join(first_store_per_stay %>% select(subject_id, stay_id, itemid, storetime),
             by = c("subject_id", "stay_id", "itemid", "storetime")) %>%
  select(-storetime)  # Remove storetime after selecting the first measurement

# Step 3: Handle multiple valuenum entries by averaging them per itemid
chartevents_filtered <- chartevents_filtered %>%
  group_by(subject_id, stay_id, itemid) %>%  # Group by stay_id and itemid
  summarize(valuenum_avg = mean(valuenum, na.rm = TRUE), .groups = "drop") %>%  # Take the average valuenum
  ungroup()

# Step 4: Convert long format to wide format to get one row per stay_id
chartevents_tble <- chartevents_filtered %>%
  pivot_wider(names_from = itemid, values_from = valuenum_avg, names_prefix = "vital_")
...
```{r}
# Create a copy of intime_year separately without modifying icustays_tble
icustays_age <- icustays_tble %>%
  mutate(intime_year = year(as.Date(intime)))  # Extract year without modifying intime

# Compute age_at_intime
age_at_intime <- icustays_age %>%
  ...
  select(subject_id, stay_id, age_at_intime)  # Keep only required columns

# Merge age_at_intime back into icustays_tble (without changing intime or other columns)
...
```{r}
# Step 1: Filter icustays_tble for adult ICU stays (intime >= 18 years old)
...
# Step 2: Merge icustays with admissions table (excluding admit_hour & admit_minute)
icu_admissions <- icustays_filtered %>%
  left_join(admissions_tble %>%
              select(-admit_hour, -admit_minute),  # Exclude these columns
            by = c("subject_id", "hadm_id"))

# Step 3: Merge with first vital measurements from chartevents_tble
icu_vitals <- icu_admissions %>%
  left_join(chartevents_tble, by = c("subject_id", "stay_id"))

# Step 4: Merge with last lab measurements before ICU stay
icu_final <- icu_vitals %>%
  left_join(labevents_tble, by = c("subject_id", "stay_id"))

# Step 5: Ensure final cohort contains one row per ICU stay
...
cat(paste0("Standard Deviation: ", summary_stats$los_sd, "\n"))
cat(paste0("Min: ", summary_stats$los_min, "\n"))
cat(paste0("Max: ", summary_stats$los_max, "\n"))

cat("\n===== Categorical Variables Summary =====\n")
cat(paste0("Number of Race Categories: ", categorical_summary$race_levels, "\n"))
cat(paste0("Number of Insurance Types: ", categorical_summary$insurance_levels, "\n"))
cat(paste0("Number of Marital Status Levels: ", categorical_summary$marital_status_levels, "\n"))
cat(paste0("Number of Gender Levels: ", categorical_summary$gender_levels, "\n"))

cat("\n===== Age at Intime Summary =====\n")
cat(paste0("Mean: ", age_intime_summary$age_mean, "\n"))
cat(paste0("Median: ", age_intime_summary$age_median, "\n"))
cat(paste0("Standard Deviation: ", age_intime_summary$age_sd, "\n"))
cat(paste0("Min: ", age_intime_summary$age_min, "\n"))
cat(paste0("Max: ", age_intime_summary$age_max, "\n"))
```
1. Boxplot of LOS versus Race: 
...
ggplot(mimic_icu_cohort, aes(x = race, y = los, fill = race)) +
  ...
  labs(title = "Length of ICU Stay vs Race", x = "Race", y = "Length of Stay (Days)")
```
...
ggplot(mimic_icu_cohort, aes(x = insurance, y = los, fill = insurance)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Length of ICU Stay vs Insurance", x = "Insurance Type", y = "Length of Stay (Days)")
```
...
```{r}
ggplot(mimic_icu_cohort, aes(x = marital_status, y = los, fill = marital_status)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Length of ICU Stay vs Marital Status", x = "Marital Status", y = "Length of Stay (Days)")
```

4. Boxplot of LOS versus Gender 
```{r}
# Boxplot: LOS vs Gender
ggplot(mimic_icu_cohort, aes(x = gender, y = los, fill = gender)) +
  geom_boxplot() +
  labs(title = "Length of ICU Stay vs Gender", x = "Gender", y = "Length of Stay (Days)")
```

5. Boplot of LOS versus Age in Time 
```{r}
# Scatter plot: LOS vs Age at ICU Admission
ggplot(mimic_icu_cohort, aes(x = age_at_intime, y = los)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", col = "blue") +
  labs(title = "Length of ICU Stay vs Age", x = "Age at ICU Admission", y = "Length of Stay (Days)")
```
...
# Boxplots: Categorizing LOS for Better Visualization
...
for (var in lab_vars) {
  print(
    ggplot(mimic_icu_cohort, aes_string(x = "los_category", y = var, fill = "los_category")) +
      geom_boxplot() +
      theme_minimal() +
      labs(title = paste("Boxplot of", var, "by LOS Category"), x = "LOS Category", y = var)
  )
}
```
...
# Categorizing LOS for Better Visualization
mimic_icu_cohort <- mimic_icu_cohort %>%
...
# Convert Data to Long Format for Faceted Boxplots
mimic_long <- mimic_icu_cohort %>%
  select(los_category, all_of(vital_vars)) %>%
  pivot_longer(cols = all_of(vital_vars), names_to = "Vital_Sign", values_to = "Value")

# Compute IQR for each vital sign to filter extreme outliers
...
# Join IQR stats to filter extreme outliers
mimic_long <- mimic_long %>%
  left_join(iqr_stats, by = "Vital_Sign") %>%
  filter(Value >= y_min & Value <= y_max)  # Keep only moderate outliers

# Define colors for each vital sign
vital_colors <- c("heart_rate" = "#E74C3C",
...
# Create a single faceted boxplot
p <- ggplot(mimic_long, aes(x = los_category, y = Value, fill = Vital_Sign)) +
  geom_boxplot(outlier.size = 1, outlier.alpha = 0.6, width = 0.6) +  # Adjust width
  stat_summary(fun = median, geom = "point", size = 3, color = "black", shape = 18) +  # Highlight Median
  scale_fill_manual(values = vital_colors) +  # Set custom colors
  theme_minimal() +
  labs(title = "Boxplot of Vital Signs by LOS Category",
...
# Summary statistics for LOS by first ICU unit
icu_unit_summary <- mimic_icu_cohort %>%
  ...
  arrange(desc(los_mean)) # Sort by mean LOS

# Print ICU Unit Summary
print(icu_unit_summary)

# Boxplot: LOS vs First ICU Unit
ggplot(mimic_icu_cohort, aes(x = first_careunit, y = los, fill = first_careunit)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Length of ICU Stay vs First ICU Unit", x = "First ICU Unit", y = "LOS (days)")

# Perform ANOVA test to check if LOS differs across ICU units
anova_result <- aov(los ~ first_careunit, data = mimic_icu_cohort)
summary(anova_result)

# If LOS distribution is not normal, use Kruskal-Wallis test (non-parametric)
