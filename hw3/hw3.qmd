...
# Define the subject_id of interest
subject_id_of_interest <- 10001217

# Filter ICU stays for the subject
subject_stays <- icustays_tble %>%
  filter(subject_id == subject_id_of_interest) %>%
  select(stay_id, intime, outtime)

# Get the relevant stay_ids
stay_ids <- subject_stays$stay_id

# Filter chartevents for the subject's ICU stays
...
# Join with d_items to get the abbreviations of vitals
chartevents_with_labels <- chartevents_filtered %>%
  inner_join(d_items %>% select(itemid, abbreviation), by = "itemid")

# Convert charttime to datetime for proper plotting
...
ggplot(chartevents_with_labels, aes(x = charttime, y = valuenum, color = abbreviation)) +
  geom_point(size = 1.2) +  # Make points more visible
  geom_line(size = 0.8) +   # Make lines smooth and readable
  facet_grid(abbreviation ~ stay_id, scales = "free_y") +  # Facet by abbreviation and stay_id
  labs(
  ...
  theme(
    ...
    panel.grid.major = element_line(size = 0.5, linetype = "dotted", color = "gray"),  # Light grid lines
...
# Merge with ICU stays to keep only relevant ICU patients
chartevents_filtered <- subset_chartevents %>%
  inner_join(icustays_tble, by = "stay_id") %>%
  filter(storetime >= intime & storetime <= outtime)  # Ensure measurements are within ICU stay

# Remove duplicate `subject_id` if both exist
chartevents_filtered <- chartevents_filtered %>%
  rename(subject_id = subject_id.x) %>%  # Keep one `subject_id`
  select(-subject_id.y)  # Remove the duplicate column

# Step 1: Identify ICU stays with at least one recorded measurement
icu_patients_with_vitals <- chartevents_filtered %>%
  select(subject_id, stay_id) %>%
  distinct()

# Step 2: Extract the first `storetime` per ICU stay
first_storetime_per_stay <- chartevents_filtered %>%
  group_by(stay_id) %>%
  summarize(first_storetime = min(storetime, na.rm = TRUE), .groups = "drop")

# Step 3: Keep all measurements recorded at the first `storetime` of each ICU stay
chartevents_first_per_stay <- chartevents_filtered %>%
  inner_join(first_storetime_per_stay, by = "stay_id") %>%
  filter(storetime == first_storetime)

# Step 4: Handle multiple vital measurements at the same `storetime` (take average)
chartevents_avg <- chartevents_first_per_stay %>%
  group_by(subject_id, stay_id, itemid) %>%
  summarize(valuenum_avg = mean(valuenum, na.rm = TRUE), .groups = "drop")

# Step 5: Reshape the data to have one row per ICU stay, keeping all items
chartevents_tble <- chartevents_avg %>%
...
chartevents_tble <- chartevents_tble %>%
  rename(
    ...
    non_invasive_bp_systolic = vital_220179,
    non_invasive_bp_diastolic = vital_220180,
    temperature_fahrenheit = vital_223761,
    respiratory_rate = vital_220210
  )

# Ensure we include only subjects who have had at least one ICU stay with vitals
chartevents_tble <- chartevents_tble %>%
  semi_join(icu_patients_with_vitals, by = "stay_id")

# Verify the number of rows
cat("Number of rows in chartevents_tble:", nrow(chartevents_tble), "\n")  # Should be 94,363
