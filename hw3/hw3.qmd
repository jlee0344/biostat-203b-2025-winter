...
It is important to note that anchor_age corresponds to patient's age in the anchor_year. If a patient's anchor_age is over 89 in the anchor_year then their ancho_age is set to 91, regardless of how old they actually are. 
...
cat("Number of rows in the result:", nrow(subset_labevents ), "\n")
print(head(subset_labevents , 10)) 
...
labevents_filtered <- subset_labevents %>%
  filter(itemid %in% lab_items) %>%  # Keep only relevant lab tests
  inner_join(icustays_tble, by = "subject_id") %>%
  filter(storetime < intime)  # Ensure measurements are before ICU stay
...
labevents_wide <- labevents_last %>%
  select(subject_id, stay_id, itemid, value) %>%
  pivot_wider(names_from = itemid, values_from = value)

colnames(labevents_wide) <- c("subject_id", "stay_id", "bicarbonate", "chloride", "creatinine",
