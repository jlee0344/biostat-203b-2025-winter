...
icustays_filtered <- icustays_tble %>%
  inner_join(patients_tble %>% select(subject_id, anchor_age), by = "subject_id") %>%
  filter(anchor_age >= 18)

# Step 2: Merge icustays with admissions table
...
expected_columns <- c("subject_id", "hadm_id", "stay_id", "first_careunit", "last_careunit",
                      "intime", "outtime", "los", "admittime", "dischtime", "deathtime",
                      "admission_type", "admit_provider_id", "admission_location", "discharge_location",
                      "insurance", "language", "marital_status", "race", "anchor_age", "anchor_year",
...
# Get actual columns in mimic_icu_cohort
actual_columns <- colnames(mimic_icu_cohort)
