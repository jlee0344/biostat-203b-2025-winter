...
```{r}
# Ensure ICD codes have a consistent format
diagnoses_info <- diagnoses_info %>%
  mutate(icd_code = str_pad(icd_code, width = 5, side = "left", pad = "0"))

# Merge diagnoses data with descriptions
diagnoses_info <- diagnoses_info %>%
  left_join(d_icd_diagnoses, by = c("icd_code", "icd_version"))

# Determine the appropriate column for diagnosis descriptions
diagnosis_col <- grep("long_title", colnames(diagnoses_info), value = TRUE)

if ("long_title" %in% diagnosis_col) {
  diagnoses_info <- diagnoses_info %>% rename(diagnosis_name = long_title)
} else if ("long_title.x" %in% diagnosis_col) {
  diagnoses_info <- diagnoses_info %>% rename(diagnosis_name = long_title.x)
} else if ("long_title.y" %in% diagnosis_col) {
  diagnoses_info <- diagnoses_info %>% rename(diagnosis_name = long_title.y)
} else {
  stop("No long_title column detected in diagnoses_info")
}

# Extract the top three most common diagnoses, excluding missing values
...
# Create summary text of the top diagnoses if available
top_diagnoses_text <- ifelse(length(top_diagnoses) > 0,
  paste(top_diagnoses, collapse = "\n"), "")

# Construct a patient summary while omitting missing values
patient_summary <- paste(
  "Patient", 
  subject_id, 
  ifelse(is.na(patient_info$gender), "", patient_info$gender), 
  ifelse(is.na(patient_info$anchor_age), "", paste(patient_info$anchor_age, "years old"))
) %>% str_squish()

# Convert timestamps in transfers_info
transfers_info <- transfers_info %>%
  mutate(
    intime = as.POSIXct(intime, format = "%Y-%m-%d %H:%M:%S"),
    outtime = as.POSIXct(outtime, format = "%Y-%m-%d %H:%M:%S")
  ) %>%
  filter(!is.na(outtime))  # Exclude records with missing outtime

# Convert timestamps in lab events
labevents_info <- labevents_info %>%
  mutate(chartdate = as.POSIXct(charttime, format = "%Y-%m-%d %H:%M:%S"))

# Merge procedures data with descriptions and handle duplicate columns
...
# Identify the correct column for procedure descriptions
procedure_col <- grep("long_title", colnames(procedures_info), value = TRUE)

if (length(procedure_col) > 1) {
  procedures_info <- procedures_info %>%
    select(-one_of(procedure_col[-1])) %>%
    rename(procedure_name = procedure_col[1])
} else if (length(procedure_col) == 1) {
  procedures_info <- procedures_info %>%
    rename(procedure_name = procedure_col[1])
} else {
  stop("No procedure_name column detected in procedures_info")
}

# Remove rows with missing procedure names
procedures_info <- procedures_info %>% filter(!is.na(procedure_name))

# Define colors for different care units
care_unit_colors <- c(
...
# Assign unique shapes to procedures
procedure_shapes <- setNames(seq(15, 15 + length(unique(procedures_info$procedure_name)) - 1),
  unique(procedures_info$procedure_name))

# Define the desired y-axis order
y_order <- factor(c("ADT", "Lab", "Procedure"), levels = c("ADT", "Lab", "Procedure"))

# Create a patient trajectory visualization
plot <- ggplot() +
  geom_segment(
    data = transfers_info,
...
# Extract the minute of admission
admissions_tble <- admissions_tble %>%
...
# Merge ICU stays with admissions and patient data
mimic_icu_cohort <- icustays_adults %>%
  left_join(admissions_tble, by = c("subject_id", "hadm_id")) %>%
  left_join(patients_tble, by = "subject_id")

# Get last lab measurement before ICU admission
last_labs <- labevents_tble %>%
  filter(charttime < intime) %>%  # Ensure measurement is before ICU stay
  group_by(subject_id, hadm_id, stay_id, itemid) %>%  # Group by patient & test type
  slice_max(charttime, n = 1) %>%  # Select the latest measurement
  ungroup()

# Merge last lab measurements into cohort
mimic_icu_cohort <- mimic_icu_cohort %>%
  left_join(last_labs, by = c("subject_id", "hadm_id", "stay_id"))


# Get first vital sign measurement during ICU stay
first_vitals <- chartevents_tble %>%
  filter(charttime >= intime) %>%  # Ensure measurement is during ICU stay
  group_by(subject_id, hadm_id, stay_id, itemid) %>%  # Group by patient & vital sign type
  slice_min(charttime, n = 1) %>%  # Select the earliest measurement
  ungroup()

# Merge first vital signs into cohort
mimic_icu_cohort <- mimic_icu_cohort %>%
  left_join(first_vitals, by = c("subject_id", "hadm_id", "stay_id"))

# Check final dataset
glimpse(mimic_icu_cohort)  # View structure
