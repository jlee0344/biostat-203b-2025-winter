...
#Delete later: The datasets that I used: 
...
Code for question 6: I didn't get the right output for temperature_farenheit 

Checking subset_chartevents for valuenum for temeprature: 

```{r}
subset_temperature <- chartevents_filtered%>%
  filter(itemid == 223761) %>%
...
```{r}
# Filter for item_id = 223761 and arrange by subject_id, stay_id
subset_temperature <- chartevents_first_per_stay %>%
  arrange(subject_id, stay_id)
...
chartevents_table <- chartevents_filtered %>%
...
chartevents_filtered <- chartevents_filtered %>%
  inner_join(first_store_per_stay %>% select(stay_id, storetime),
             by = c("stay_id", "storetime"))

chartevents_filtered <- chartevents_filtered %>%
  group_by(subject_id, stay_id, storetime, itemid) %>%
