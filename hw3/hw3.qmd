...
# Create a histogram of ICU stays per patient with improved visualization
ggplot(icu_stays_per_subject, aes(x = num_stays)) +
  geom_histogram(binwidth = 1, fill = "steelblue", color = "black", alpha = 0.8) +
  scale_x_continuous(breaks = seq(1, min(40, max(icu_stays_per_subject$num_stays)), by = 1),
                     limits = c(1, min(40, max(icu_stays_per_subject$num_stays)))) +
  scale_y_log10() +  # Apply log scale to better show variations
  labs(title = "Distribution of ICU Stays per Patient",
       x = "Number of ICU Stays per Patient",
