...
```{r}
# Step 3: Create new variable los_long (TRUE if LOS >= 2 days)
mimic_icu_cohort_2 <- mimic_icu_cohort_2 %>%
...
```{r}
# Step 4: Summarize data using tbl_summary(), stratified by los_long
summary_table <- mimic_icu_cohort_2 %>%
  select(first_careunit, last_careunit, admission_type, admission_location, discharge_location, race, los_long) %>%
...
Consolidating Graph for 1.1 

Retrieving all the datasets...
