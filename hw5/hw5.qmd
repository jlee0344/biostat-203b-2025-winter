...
#Approach 1: Logistic Regression with e-net regularization
...
train_sampled <- train_data %>% sample_frac(0.10) 
...
# DEBUG: Check structure of rf_pred
print(str(rf_pred))
...
# Ensure test_predictions is a vector
test_predictions <- as.numeric(test_predictions)
print(length(test_predictions))  # Should be same as nrow(test_data)
...
print(best_threshold)  # Should be a single value

# Convert probabilities to binary predictions
test_pred_labels <- ifelse(test_predictions > best_threshold, "Class1", "Class0")

# Ensure test_pred_labels is a character vector
test_pred_labels <- as.character(test_pred_labels)
print(length(test_pred_labels))  # Must be 47223

# Ensure test_pred_labels is a factor with correct levels
...
# Debug length check before confusion matrix
print(length(test_pred_labels))  # Must be 47223
print(length(test_data$los_long))  # Must be 47223
...
# Feature Importance
print(importance(final_rf))  # Display feature importance scores
```
